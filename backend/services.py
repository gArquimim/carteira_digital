...
def get_statement(user_id):
    ...
    statement = []
    for type_db, value_db, date_db in historic:
        type = str(type_db).title()
        date = date_db.strftime("%d/%m/%Y %H:%M:%S")
        value = f"-R${value_db:.2f}" if type_db == "expense" else f"+R${value_db}"
